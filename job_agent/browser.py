...
async def main_with_jobs(llm, job_urls):
    """Open browser tabs for multiple job URLs"""
    config = BrowserConfig(chrome_instance_path="/usr/bin/google-chrome-stable")
    browser = Browser(config=config)

    print(f"🌐 Opening {len(job_urls)} job tabs...")

    # Create initial actions to open all job URLs in separate tabs
    initial_actions = []
    ...
    task = (
        "You have opened multiple job application tabs. "
        "Switch between tabs to review each job posting. "
        "For each job that looks suitable, look for 'Easy Apply' or 'Apply' buttons. "
        "Before applying to any job, ask the human for confirmation. "
        "When ready to apply, fill forms using candidate CV information. "
        "Always defer to human approval before submitting applications."
    )
